...
def get_leaderboard(sort='kills', limit=50) -> list:
    ...
    if sort == 'kda_worst':
        rows = db.execute(
            '''SELECT *, CAST(kills AS REAL) / CASE WHEN deaths = 0 THEN 1 ELSE deaths END AS kd_ratio
               FROM player_stats WHERE error IS NULL AND name != "" AND kills >= 1 AND deaths >= 50
    ...
    if sort == 'charges':
        rows = db.execute(
            '''SELECT *,
               CAST(lamps AS REAL) / golems AS charge_rate
               FROM player_stats
               WHERE error IS NULL AND name != "" AND golems >= 1 AND lamps >= 50
                 AND CAST(lamps AS REAL) / golems <= 1.0
